...
export default function OrgSelector({
  ...
  useEffect(() => {
    const fetchOrgs = async () => {
      ...
      try {
        ...
        setError('Could not load orgs. Showing mock orgs.')
  ...
  const OrgOption = ({ org }: { org: SalesforceOrg }) => (
  ...
  return (
    <div className="bg-gray-800 border border-gray-700 rounded-xl p-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select
            ...
            className="w-full bg-gray-900 border border-gray-600 text-white rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-sf-blue focus:ring-1 focus:ring-sf-blue disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {fetching ? (
              <option>Loading orgs…</option>
            ) : (
              orgs.map(org => <OrgOption key={org.alias} org={org} />)
            )}
          </select>
          {selectedSource && (
            <div className="mt-1.5 text-xs text-gray-500 truncate">
              <span className="text-gray-400">URL:</span> {selectedSource.instanceUrl}
            </div>
          )}
        </div>
        ...
        <div>
          ...
          {selectedTarget && (
            <div className="mt-1.5 text-xs text-gray-500 truncate">
              <span className="text-gray-400">URL:</span> {selectedTarget.instanceUrl}
            </div>
          )}
        </div>
      </div>
      ...
      <div className="mt-4 flex items-center gap-3">
        <button
          onClick={onCompare}
          disabled={loading || fetching || !sourceOrg || !targetOrg || sourceOrg === targetOrg}
          className="flex items-center gap-2 px-5 py-2.5 bg-sf-blue hover:bg-sf-blue-dark text-white font-semibold rounded-lg transition-colors duration-150 disabled:opacity-50 disabled:cursor-not-allowed text-sm"
        >
          {loading ? (
            <>
